...
const MovieInformation = () => {
    ...
    const addToFavorites = async () => {
        await axios.post(`https://api.themoviedb.org/3/account/${user.id}/favorite?api_key=0aeca8b1291c3a641cadf8ba31b16c73&session_id=${localStorage.getItem('session_id')}`,
            {
                media_type: 'movie',
                media_id: id,
    ...
    const addToWatchlist = async () => {
        await axios.post(`https://api.themoviedb.org/3/account/${user.id}/watchlist?api_key=0aeca8b1291c3a641cadf8ba31b16c73&session_id=${localStorage.getItem('session_id')}`,
            {
                media_type: 'movie',
                media_id: id,
    ...
    return (
        <Grid container className={classes.containerSpaceAround}>
                ...
                <Grid item className={classes.containerSpaceAround}>
                    <Box display="flex" align="center">
                        ...
                        <Typography variant='subtitle1' gutterBottom style={{
                            marginLeft: "10px"
                        }}>{data?.vote_average} / 10</Typography>
                    </Box>
                ...
                <Grid item container spacing={2}>
                    {data && data.credits.cast?.map((character, i) => (
                        ...
                        character.profile_path && (
                            <Grid key={i} item xs={4} md={2} component={Link} to={`/actors/${character.id}`} style={{
                                textDecoration: "none"
                            }}>
            ...
            <Box marginTop="5rem" width="100%">
                ...
                {recommendations ? <MovieList movies={recommendations} numberOfMovies={12} /> : <Box>Sorry, no recommended movies was found.</Box>}
            </Box>
